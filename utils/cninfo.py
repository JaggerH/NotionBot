...
class cninfo:
    def __init__(self, max_threads=5, skip_download_stock_json=False):
        ...
        self.logger.setLevel(logging.CRITICAL)  # 设置日志级别为CRITICAL
        self.column_to_market = {
            "szse": "szse",
            "hke": "hke",
            "gfzr": "third",
            "fund": "fund",
            "bond": "bond",
        }  # key是json的地址，value是对应的market查询参数
    ...
    def get_stock_json(self, columns: dict) -> dict:
        ...
        for column in columns:
            self.logger.info(
                f"请求 http://www.cninfo.com.cn/new/data/{column}_stock.json 中..."
            )
            stock_json = client.get(
                f"http://www.cninfo.com.cn/new/data/{column}_stock.json"
            ).json()
            stockList = stock_json["stockList"]
            self.logger.info(f"请求成功，共有{len(stockList)}条记录")
            market_to_stocks[self.column_to_market[column]] = {
                each["code"]: each for each in stockList
            }
    ...
    def query_announcements_info(self, filter: dict, download_pdf=False) -> list:
        ...
        payload = {
            ...
            "stock": ";".join(
                [i + "," + valid_stock_info[i] for i in valid_stock_info]
            ),
    ...
    def start_download_announcements_pdf(self, announcements_info_list: list) -> None:
        ...
        with concurrent.futures.ThreadPoolExecutor(
            ...
            future_to_url = {
                executor.submit(
                    self.download_announcements_pdf, announcement, client
                ): announcement
                for announcement in announcements_info_list
            }
            for future in concurrent.futures.as_completed(future_to_url):
                # _ = future.result()
                announcement = future_to_url[future]
                path = future.result()
                print(path)
                if path:
                    announcement['path'] = path

    def download_announcements_pdf(
        self, announcement: dict, client: httpx.Client
    ) -> None:
